...
interface ProjectsScreenProps {
  projects?: Array<{ id: string; name: string; status: string; thumbnail?: string | null }>;
  onCreateProject?: () => void;
}

type SimplifiedStatus = 'review' | 'progress' | 'complete';

interface StatusConfig {
  label: string;
  statusType: SimplifiedStatus;
  badgeClass: string;
  dotClass: string;
}

function getStatusConfig(status: string): StatusConfig {
  // Map actual project statuses to simplified status labels
  if (status === 'in_review' || status === 'awaiting_client') {
    return {
  ...
  // Default to progress for unknown statuses
  return {
    label: 'In Progress',
    statusType: 'progress',
    badgeClass: 'bg-blue-100 text-blue-700',
...
function getProjectThumbnail(thumbnail?: string | null): string {
  if (thumbnail) {
    return thumbnail;
  }
  // Use a placeholder image - you can replace this with a default image URL
    ...
    <div className="px-5 py-6">
      ...
      <div className="space-y-3">
        {projects.map((project) => {
          ...
          const thumbnail = getProjectThumbnail(project.thumbnail);
            ...
            >
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0 py-0.5">
                  ...
                  {/* Status Badge */}
                  <span
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${statusConfig.badgeClass}`}
                  >
