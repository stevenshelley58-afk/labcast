...
export default function LoginPage() {
    ...
    try {
      ...
      const redirectTo =
        typeof window !== 'undefined'
          ? `${window.location.origin}/render-vault/dashboard`
          : 'https://labcast.com.au/render-vault/dashboard';
  ...
  const handlePasswordLogin = async () => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        window.location.href = '/render-vault/dashboard';
      }
    } catch (err) {
      setStatus('error');
      setMessage('Something went wrong. Please try again.');
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center gap-6 px-6 text-center bg-bg-paper">
      <div className="space-y-2">
        <h1 className="text-2xl font-semibold tracking-tight text-text-ink">Login to Render Vault</h1>
